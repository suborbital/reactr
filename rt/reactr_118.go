...
type JobFunc[T any, R any] func(T) *Result[Output]
...
func (r *Reactr) Do(job Job[Input, Output]) *Result[Output] {
...
func (r *Reactr) DoWithCaps(job Job[Input, Output], caps Capabilities) *Result[Output] {
...
func (r *Reactr) Schedule(s Schedule[Input, Output]) {
	r.core.watch(s)
}

// Register registers a Runnable with the Reactr and returns a shortcut function to run those jobs
func (r *Reactr) Register(jobType string, runner Runnable[Input, Output], options ...Option) JobFunc[Input, Output] {
	r.RegisterWithCaps(jobType, runner, r.defaultCaps, options...)

	helper := func(data Input) *Result[Output] {
		return r.Do(NewJob[Input, Output](jobType, data))
	}
...
func (r *Reactr) RegisterWithCaps(jobType string, runner Runnable[Input, Output], caps Capabilities, options ...Option) {
	caps.doFunc = r.core.do
...
// 	helper := func(data T) *Result[Output] {
// 		job := NewJob[Input, Output](msgType, data)
...
func (r *Reactr) Job(jobType string, data any) Job[Input, Output] {
	return NewJob[Input, Output](jobType, data)
}
